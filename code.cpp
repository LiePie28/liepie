...
class heapTree{
	...
	public:
		...
		void heapInsertMinMax( hT& newItem){
		
			item.push_back(newItem) ;
			int place = item.size() - 1 ;
			int parent = (place-1)/2;
			int level = log2(item.size()) + 1  ;
			if( level != 1 ){
				if((level % 2) != 0 ) {
				
				    if(item.at(place).number > item.at(parent).number){
				    	Swap(item, place, parent) ;	
						place = parent ;
						parent = (place-1)/2 ;
						parent = (parent-1)/2 ;
						level = level - 1 ;
							...
							while((parent >= 0 ) && ( item.at(place).number > item.at(parent).number) && level > 2 ){
								Swap(item, place, parent) ;
								place = parent ;
								parent = (place-1)/2 ;
								parent = (parent-1)/2 ;
...
int main(int argc, char** argv) {
    ...
    while(command!=0){	
		...
		m2check = false ;
		cout << "*********************************************" << endl ;
        cout << "**             HeapSort Tree               **" << endl ;
        cout << "* 0. Quit                                   *" << endl ;
        cout << "* 1. Create min-Heap Trees                  *" << endl ;
        cout << "* 2. Create min-max Heap Trees              *" << endl ;
        cout << "*********************************************" << endl ;
